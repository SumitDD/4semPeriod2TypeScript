...
describe("### Describe the Friend Endpoints (/api/friends) ###", function () {
    ...
    describe("While attempting to add a user", function () {
        it("it should Add the user Jan Olsen", async () => {
            ...
            const response = await request.post('/api/friends/addfriend').send(newFriend)
            expect(response.status).to.equal(200)
            expect(response.body.id).to.be.not.null
        })

        it("It should fail to Add user due to wrong password length", async () => {
            const newFriend = { firstName: "sumit", lastName: "dey", email: "sumit@b.dk", password: "s" }
            const response = await request.post('/api/friends/addfriend').send(newFriend)
    ...
    describe("While logged in as a user", function () {
        it("It should return the logged in user", async () => {
            const response = await request.get('/api/friends/me')
                .auth("pp@b.dk", "secret")
            expect(response.status).to.equal(200)

        })
        it("It should edit the logged in user", async () => {
            const editedFriend = { firstName: "Peter", lastName: "dey", email: "pp@b.dk", password: 'secret' }
            const response = await request.put('/api/friends/editme').auth("pp@b.dk", "secret").send(editedFriend)
            expect(response.status).to.equal(200)
    ...
    describe("While verifying the get any user, given a userId (email)", function () {
        it("It should allow an admin user to find Donald Duck", async () => {
            ...
            const response = await request.get('/api/friends/find-user/dd@b.dk')
                .auth("aa@a.dk", "secret")
            expect(response.status).to.equal(200)
        })
        it("It should not, allow admin-users to find a non-existing user", async () => {
            const response = await request.get('/api/friends/find-user/111@b.dk')
                .auth("aa@a.dk", "secret")
    ...
    describe("While verifying the 'edit any user', given a userId (email)", function () {
        it("It should allow an admin-user to edit Peter Pan", async () => {
            const editedFriend = { firstName: "Peter", lastName: "Hansen", email: "pp@b.dk", password: 'secret' }
            const response = await request.put('/api/friends/pp@b.dk').auth("aa@a.dk", "secret").send(editedFriend)
            expect(response.status).to.equal(200)

        })
        it("It should NOT allow a non-admin user to edit Peter Pan", async () => {
            const editedFriend = { firstName: "Peter", lastName: "Pan2", email: "pp@b.dk", password: 'secret' }
            const response = await request.put('/api/friends/pp@b.dk').auth("dd@b.dk", "secret").send(editedFriend)
            expect(response.status).to.equal(401)
        })
    })
    describe("While verifying the delete any user, given a userId (email)", function () {
        it("It should allow an admin user to delete Donald Duck", async () => {
            ...
            const response = await request.delete('/api/friends/delete/pp@b.dk').auth("aa@a.dk", "secret")
            expect(response.status).to.equal(200)
